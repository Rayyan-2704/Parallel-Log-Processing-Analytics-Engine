    ...
    while (pos < file_size && next_seg <= n) {
        ...
        for (ssize_t j = 0; j < r; j++) {
            ...
            if (rbuf[j] == '\n') {
                seg_lines++;
                /*
                 * Has pos crossed the raw boundary for next_seg?
                 * If so, commit this newline as the alignment point.
                 *
                 * Use `if` not `while`: only ONE boundary can be committed
                 * per newline. A `while` loop here would reset seg_lines
                 * multiple times for a single newline, causing the triggering
                 * newline to be credited to the previous segment AND
                 * shifting every subsequent segment's line_offset by -1.
                 *
                 * Edge case: two raw boundaries fall between the same pair of
                 * newlines (file has very long lines). The inner flush loop
                 * after the read handles those by collapsing them to the same
                 * alignment point (file_size or the next newline found).
                 */
                if (next_seg <= n - 1 && pos >= raw[next_seg]) {
