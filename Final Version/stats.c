...
void stats_init(GlobalStats *gs, int num_patterns)
{
    ...
    for (int i = 0; i < num_patterns; i++) {
        gs->pattern_first_line[i] = 0;   // not matched yet
    }
    pthread_mutex_init(&gs->lock, NULL);
}

/* Two-phase merge: caller (worker thread) holds its local stats, acquires the global mutex once, then adds everything atomically. */
void stats_merge(GlobalStats *gs, const LocalStats *ls, int num_patterns)
{
    ...
    gs->truncated_lines += ls->truncated_lines;
