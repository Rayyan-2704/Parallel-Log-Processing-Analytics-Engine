...
void *worker_thread(void *arg)
{
    ...
    while (ptr < end) {
        if (*ptr == '\n') {
            /* Process line [line_start, ptr).
             *
             * Blank-line fix: both branches fall through to the SAME
             * local_line++ and line_start update at the bottom of this
             * if (*ptr=='\n') block, then ptr++ at the bottom of the while
             * advances by exactly one byte.  The old blank-line branch had
             * its own local_line++ AND line_start update followed by
             * continue, then the outer block ran them again — double-counting
             * the line, and the explicit ptr++ inside the branch caused a
             * double-advance that silently dropped the byte after every blank
             * line. */
            ...
            if (line_len > 0) {
                ...
                *ptr = saved;
                ...
                /* Genuine blank line — count it, no parsing needed.
                 * No ptr++ here; fall through to the shared updates below. */
                ls.total_lines++;
                if (wa->progress_lines)
                    atomic_fetch_add(wa->progress_lines, 1);
            }

            /* Shared bookkeeping for BOTH non-blank and blank lines.
             * This runs once per '\n', making ptr++ at loop bottom the
             * sole advance — no double-step possible. */
            local_line++;
            line_start = ptr + 1;
        }
        ptr++;   /* the only ptr++ in the loop */
    }
    ...
    /* Assign before free(buf) — truncated is a stack variable, not inside
     * buf, but placing this after free() looks like use-after-free on a
     * quick audit read.  Moving it here removes any possible confusion. */
