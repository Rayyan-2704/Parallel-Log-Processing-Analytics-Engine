    ...
    while (pos < file_size && next_seg <= n) {
        ...
        for (ssize_t j = 0; j < r; j++) {
            ...
            if (rbuf[j] == '\n') {
                seg_lines++;
                /*
                 * Commit all raw boundaries that this newline's pos satisfies.
                 *
                 * `while` not `if`: when a line is longer than file_size/n,
                 * two or more raw[] entries can fall between the same pair of
                 * newlines.  With `if`, only the first was committed here; the
                 * rest deferred to the end-of-file flush which set them to
                 * file_size, producing zero-byte segments whose workers
                 * silently emit no output.
                 *
                 * line_offset correctness: seg_lines is reset to 0 on the
                 * first commit.  Every additional boundary committed at the
                 * same pos gets line_offsets[k] = line_offsets[k-1] + 0 —
                 * correct, because those segments contain no lines of their own.
                 */
